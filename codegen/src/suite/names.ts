...
const currency = (cldr: CLDR, id: string) =>
  cldr.Numbers.getCurrencyDisplayName(id as CurrencyType, {
    context: 'begin-sentence',
  });

const currencyPlural = (cldr: CLDR, id: string, num: Decimal) =>
  cldr.Numbers.getCurrencyPluralName(num, id as CurrencyType, {
    context: 'begin-sentence',
  });

const language = (cldr: CLDR, id: string) =>
  cldr.General.getLanguageDisplayName(id, {
    context: 'begin-sentence',
    type: 'long',
  });

const script = (cldr: CLDR, id: string) =>
  cldr.General.getScriptDisplayName(id, {
    context: 'begin-sentence',
    type: 'long',
  });

const region = (cldr: CLDR, id: string) =>
  cldr.General.getRegionDisplayName(id, {
    context: 'begin-sentence',
    type: 'long',
  });
...
const FORMATTERS: { [key: string]: [keyof SchemaConfig, Formatter] } = {
  ...
  'currency-plural-0': [
    'currency-id',
    (c: CLDR, id: string) => currencyPlural(c, id, ZERO),
  ],
  'currency-plural-1': [
    'currency-id',
    (c: CLDR, id: string) => currencyPlural(c, id, ONE),
  ],
  'currency-plural-2': [
    'currency-id',
    (c: CLDR, id: string) => currencyPlural(c, id, TWO),
  ],
